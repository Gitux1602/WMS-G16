...
from app.models import BaseDatosEnum, CodigoBarras, Inventario, InventarioDetalle, Ubicaciones, UBICACIONES_PREDEFINIDAS
...
def mostrar_conteo_sap(docnum):
    ...
    """
    print("UBICACIONES_PREDEFINIDAS:", UBICACIONES_PREDEFINIDAS)  # Depuración
...
def procesar_codigo_qr():
    ...
    try:
        ...
        # Obtener el documento de inventario actual (puedes pasar el docnum desde el frontend)
        docnum = data.get('docnum')  # Asegúrate de enviar el docnum desde el frontend
        inventario = Inventario.query.get_or_404(docnum)
        ...
        return jsonify({
            ...
            "itemcode": itemcode,
            "descripcion": detalle.articulo.descripcion,  # Descripción del artículo
            "sumatoria_cantidad": sumatoria_cantidad,
            "cantidad_ubicacion": cantidad_ubicacion,  # Cantidad contada en la ubicación
            "diferencias": detalle.diferencias
        })
    except Exception as e:
        db.session.rollback()
